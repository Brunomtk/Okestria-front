...
export function createGatewayProxy(options) {
  ...
  wss.on("connection", (browserWs, req) => {
    ...
    const forwardConnectFrame = (frame) => {
      ...
      const connectFrame = browserHasAuth
        ...
        : {
            ...frame,
            params: (() => {
              const nextParams = injectAuthToken(frame.params, upstreamToken);
              if (isObject(nextParams.device)) {
    ...
    const startUpstream = async () => {
      try {
        const settings = await loadUpstreamSettings(req);
        ...
        pendingUpstreamSetupError = {
          ...
          message: `Failed to load Studio gateway settings. ${details}`,
        };
        return;
      }
      ...
      upstreamWs.on("error", (err) => {
        ...
        sendConnectError(
          ...
          `Failed to connect to upstream gateway WebSocket. ${details}`
        );
      });
