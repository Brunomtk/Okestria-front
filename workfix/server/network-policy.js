...
export const resolveHosts = (env = process.env) => {
  const host = String(env.HOST ?? "").trim();
  if (host) return [host];
  return ["127.0.0.1", "::1"];
};

export const resolveHost = (env = process.env) => {
...
export const isPublicHost = (host) => {
...
export const assertPublicHostAllowed = ({ host, studioAccessToken }) => {
