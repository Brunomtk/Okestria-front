...
const isJanitorRenderAgent = (agent: RenderAgent): agent is JanitorRenderAgent =>
  "role" in agent && agent.role === "janitor";
type RoamPointInput = { x: number; y: number } | [number, number];

const toRoamPoint = (point: RoamPointInput) =>
  Array.isArray(point) ? { x: point[0], y: point[1] } : point;

const resolveRoamPoints = (agentId: string) =>
  (isRemoteOfficeAgentId(agentId) ? REMOTE_ROAM_POINTS : ROAM_POINTS).map(toRoamPoint);

const chooseRoamPoint = (agentId: string) => {
  const points = resolveRoamPoints(agentId);
...
export function useRebuiltAgentTick(
  ...
  const tick = (delta = 1 / 60) => {
    ...
    const moved: RenderAgent[] = renderAgentsRef.current.map((agent): RenderAgent => {
      const isJanitor = isJanitorRenderAgent(agent);
