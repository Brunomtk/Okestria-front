...
export const deriveHydrateAgentFleetResult = (
  ...
  if (input.statusSummary && input.previewResult) {
    ...
    if (sessionKeys.length > 0) {
      ...
      const defaultAgentId = input.agentsResult.defaultId?.trim() || null;
      let bestAgentId: string | null =
        defaultAgentId && seeds.some((seed) => seed.agentId === defaultAgentId)
          ? defaultAgentId
          : seeds[0]?.agentId ?? null;
      let bestTs = bestAgentId ? assistantAtByAgentId.get(bestAgentId) ?? 0 : 0;
      for (const seed of seeds) {
        const ts = assistantAtByAgentId.get(seed.agentId) ?? 0;
        if (seed.agentId === defaultAgentId && ts === 0) {
