...
export async function hydrateAgentFleetFromGateway(params: {
  ...
  const allowedAgentSlugs = new Set(
    (params.allowedAgentSlugs ?? [])
  ...
  agentsResult = {
    ...agentsResult,
    agents: agentsResult.agents.filter((agent) => {
      if (isTemporarySkillAgentName(agent.name ?? agent.identity?.name)) {
        return false;
      }
      if (!hasAllowedAgentFilter) {
        return true;
      }
      const trimmedId = agent.id?.trim() ?? "";
      if (trimmedId && allowedAgentIds.has(trimmedId)) {
        return true;
      }
      const candidateSlug = slugifyGatewayAgentName(agent.name ?? agent.identity?.name ?? "");
      if (candidateSlug && allowedAgentSlugs.has(candidateSlug)) {
        return true;
      }
      return false;
    }),
