...
const TAB_LABELS: Record<Exclude<HQSidebarTab, "analytics">, string> = {
  inbox: "Inbox",
  history: "History",
  playbooks: "Playbooks",
};

const PRIMARY_TABS: Exclude<HQSidebarTab, "analytics">[] = ["inbox", "history", "playbooks"];
...
export function HQSidebar({
  ...
  const resolvedTab = activeTab === "analytics" ? "inbox" : activeTab;
  const activePanel =
    resolvedTab === "inbox"
      ? inboxPanel
      : resolvedTab === "history"
  ...
  return (
    <aside className="pointer-events-none fixed inset-y-0 right-0 z-20 flex justify-end">
      ...
      {open ? (
        <div className={`pointer-events-auto flex h-full ${panelWidthClass} flex-col border-l border-cyan-500/20 bg-black/85 shadow-2xl backdrop-blur`}>
          ...
          <div
            role="tabpanel"
            id={`hq-panel-${resolvedTab}`}
            aria-labelledby={`hq-tab-${resolvedTab}`}
