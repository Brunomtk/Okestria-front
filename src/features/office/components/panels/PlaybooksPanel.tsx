...
export function PlaybooksPanel({
  ...
  const enabledJobsCount = jobs.filter((job) => job.enabled).length;
  const selectedAgentName = selectedAgentId
    ? agentById.get(selectedAgentId)?.name || selectedAgentId
    : "No agent selected";
    ...
    <section className="flex h-full min-h-0 flex-col">
      <div className="border-b border-cyan-500/10 px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 max-w-2xl font-mono text-[11px] leading-5 text-white/40">
              Keep recurring automations, standups, and quick launch templates in one cleaner control room.
            </div>
          </div>
          <button
            type="button"
            onClick={() => void loadJobs()}
            className="rounded border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-200 transition-colors hover:border-cyan-400/40 hover:text-cyan-100"
          >
            Refresh jobs
          </button>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded border border-cyan-500/15 bg-cyan-500/[0.05] px-3 py-3">
            <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-200/85">Active jobs</div>
            <div className="mt-2 font-mono text-2xl text-white/90">{activeJobsCount}</div>
            <div className="mt-1 font-mono text-[11px] text-white/45">{loading ? "Refreshing list..." : `${enabledJobsCount} enabled right now`}</div>
          </div>
          <div className="rounded border border-emerald-500/15 bg-emerald-500/[0.05] px-3 py-3">
            <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-emerald-200/85">Standup schedule</div>
            <div className="mt-2 font-mono text-[13px] uppercase tracking-[0.14em] text-white/85">
              {standupScheduleEnabled ? "Enabled" : "Manual only"}
            </div>
            <div className="mt-1 font-mono text-[11px] text-white/45">{standupCronExpr || "0 9 * * 1-5"} · {standupTimezone || "UTC"}</div>
          </div>
          <div className="rounded border border-white/10 bg-white/[0.03] px-3 py-3">
            <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/55">Quick launch</div>
            <div className="mt-2 font-mono text-[13px] uppercase tracking-[0.14em] text-white/85">{PLAYBOOK_TEMPLATES.length} templates ready</div>
            <div className="mt-1 font-mono text-[11px] text-white/45">Select a template, pick an agent, and launch.</div>
          </div>
        </div>

        <div className="mt-4 rounded border border-cyan-500/10 bg-black/20">
          <div className="flex items-center justify-between gap-2 border-b border-cyan-500/10 px-3 py-3">
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/45">Running now</div>
              <div className="mt-1 font-mono text-[11px] text-white/55">See what is active, trigger it again, or remove it.</div>
            </div>
          </div>
          ...
          <div className="px-3 py-3">
            {loading ? (
              ...
              <div className="rounded border border-dashed border-white/10 bg-white/[0.02] px-3 py-4 font-mono text-[11px] text-white/35">
                No playbooks started yet. Use the quick launch area below to create the first one.
              </div>
            ) : (
              <div className="grid gap-3 xl:grid-cols-2">
                {jobs.map((job) => {
                  ...
                  return (
                    <div key={job.id} className="rounded border border-white/8 bg-white/[0.03] px-3 py-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="truncate font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-white/85">{job.name}</div>
                          <div className="mt-1 font-mono text-[11px] text-white/45">Assigned to {agentName}</div>
                        </div>
                        <div className="shrink-0 rounded border border-cyan-500/20 bg-cyan-500/10 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-[0.12em] text-cyan-200">{job.state.lastStatus ?? "ready"}</div>
                      </div>

                      <div className="mt-3 grid gap-2 sm:grid-cols-3">
                        <div className="rounded border border-white/8 bg-black/20 px-2 py-2">
                          <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/35">Schedule</div>
                          <div className="mt-1 font-mono text-[11px] leading-5 text-white/70">{formatCronSchedule(job.schedule)}</div>
                        </div>
                        <div className="rounded border border-white/8 bg-black/20 px-2 py-2">
                          <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/35">Next run</div>
                          <div className="mt-1 font-mono text-[11px] leading-5 text-white/70">{formatRelativeDateTime(job.state.nextRunAtMs)}</div>
                        </div>
                        <div className="rounded border border-white/8 bg-black/20 px-2 py-2">
                          <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-white/35">Last run</div>
                          <div className="mt-1 font-mono text-[11px] leading-5 text-white/70">{formatRelativeDateTime(job.state.lastRunAtMs)}</div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button
                          ...
                          className="rounded border border-amber-500/25 bg-amber-500/10 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-amber-200 transition-colors hover:border-amber-400/50 hover:text-amber-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                        ...
                        <button
                          ...
                          className="rounded border border-rose-500/25 bg-rose-500/10 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-rose-200 transition-colors hover:border-rose-400/50 hover:text-rose-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          {deleteBusyJobId === job.id ? "Deleting" : "Remove"}
                        </button>
                      </div>
                    </div>
                  );
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[1.25fr_1fr]">
          <div className="rounded border border-emerald-500/15 bg-emerald-500/[0.05] px-3 py-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-emerald-200/85">Automated standup</div>
                <div className="mt-1 max-w-2xl font-mono text-[11px] leading-5 text-white/50">
                  Configure the daily meeting in a simpler way. Quick options stay visible, advanced setup is tucked below.
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className="rounded border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-emerald-100 transition-colors hover:border-emerald-400/50 hover:text-white"
                >
                  Start now
                </button>
                <button
                  ...
                  className="rounded border border-cyan-500/25 bg-cyan-500/10 px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-100 transition-colors hover:border-cyan-400/50 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {standup.saving ? "Saving" : "Save settings"}
                </button>
              </div>
            </div>

            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <label className="rounded border border-white/8 bg-black/20 px-3 py-3">
                ...
                <span className="mt-2 block font-mono text-[11px] text-white/40">
                  Leave off if you only want to trigger meetings manually.
                </span>
              </label>
              ...
              <label className="rounded border border-white/8 bg-black/20 px-3 py-3">
                ...
                <span className="mt-2 block font-mono text-[11px] text-white/40">
                  Open the standup board automatically when the meeting starts.
                </span>
              </label>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {schedulePresets.map((preset) => {
                const isActive = standupCronExpr.trim() === preset.value;
                return (
                  <button
                    key={preset.value}
                    type="button"
                    onClick={() => setStandupCronExpr(preset.value)}
                    className={`rounded border px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors ${
                      isActive
                        ? "border-cyan-400/35 bg-cyan-500/10 text-cyan-100"
                        : "border-white/10 bg-black/20 text-white/55 hover:border-cyan-400/30 hover:text-cyan-100"
                    }`}
                  >
                    {preset.label}
                  </button>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <label className="flex flex-col gap-1 md:col-span-2">
                <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Schedule</span>
                <input
              ...
              <label className="flex flex-col gap-1">
                <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Timezone</span>
                <input
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <label className="flex flex-col gap-1">
                <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Seconds per speaker</span>
                <input
              ...
              <label className="rounded border border-white/8 bg-black/20 px-3 py-3 md:col-span-2">
                <span className="flex items-center gap-2 font-mono text-[11px] text-white/80">
                  <input
                    type="checkbox"
                    checked={jiraEnabled}
                ...
                <span className="mt-2 block font-mono text-[11px] text-white/40">
                  Turn this on only if your standup should pull tasks from Jira automatically.
                </span>
              </label>
            </div>

            <details className="mt-4 rounded border border-white/10 bg-black/20 px-3 py-3" open={jiraEnabled}>
              <summary className="cursor-pointer list-none font-mono text-[10px] uppercase tracking-[0.16em] text-white/55">
                Jira source settings
              </summary>
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                <label className="flex flex-col gap-1 md:col-span-2">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Jira base URL</span>
                  <input
                    value={jiraBaseUrl}
                ...
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Jira email</span>
                  <input
                    value={jiraEmail}
                ...
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Jira API token</span>
                  <input
                    type="password"
                    value={jiraApiToken}
                    onChange={(event) => {
                    ...
                    placeholder={jiraApiTokenConfigured ? "Stored on Studio host. Enter to replace." : ""}
                    className="rounded border border-white/10 bg-black/50 px-2 py-2 font-mono text-[11px] text-white/80 outline-none"
                  />
                  {jiraApiTokenConfigured ? <span className="text-[10px] text-white/45">A Jira API token is already stored on the Studio host.</span> : null}
                </label>
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Jira project key</span>
                  <input
                ...
                <label className="flex flex-col gap-1 md:col-span-2">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Jira JQL override</span>
                  <textarea
                    value={jiraJql}
            ...
            <details className="mt-4 rounded border border-white/10 bg-black/20 px-3 py-3" open>
              <summary className="cursor-pointer list-none font-mono text-[10px] uppercase tracking-[0.16em] text-white/55">
                Manual board input
              </summary>
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Agent</span>
                  <select
                ...
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Jira assignee hint</span>
                  <input
                ...
                <label className="flex flex-col gap-1 md:col-span-2">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Current task</span>
                  <input
                    value={manualTask}
                ...
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Blockers</span>
                  <textarea
                ...
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Manual note</span>
                  <textarea
                    value={manualNote}
              ...
              <button
                ...
                className="mt-3 rounded border border-cyan-500/25 bg-cyan-500/10 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-cyan-100 transition-colors hover:border-cyan-400/50 hover:text-white"
              >
                Save manual notes
              </button>
            </details>
            ...
            {standup.meeting ? (
              <div className="mt-4 rounded border border-white/8 bg-white/[0.03] px-3 py-3 font-mono text-[11px] text-white/65">
                ...
                <div>Current speaker: {standup.meeting.currentSpeakerAgentId ?? "Waiting"}</div>
              </div>
            ) : null}
          </div>
          ...
          <div className="rounded border border-cyan-500/15 bg-cyan-500/[0.05] px-3 py-3">
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-cyan-200/85">Quick launch templates</div>
              <div className="mt-1 font-mono text-[11px] leading-5 text-white/50">
                Pick a ready-made job, choose the agent, and launch. You can still rename it before creating.
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {PLAYBOOK_TEMPLATES.map((template) => {
                ...
                return (
                  ...
                  >
                    <div className="font-mono text-[11px] font-semibold uppercase tracking-[0.16em] text-white/85">{template.name}</div>
                    <div className="mt-1 font-mono text-[11px] leading-5 text-white/50">{template.description}</div>
                  </button>
                );
              })}
            </div>

            <div className="mt-4 rounded border border-white/8 bg-black/20 px-3 py-3">
              <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/45">Selected template</div>
              <div className="mt-2 font-mono text-[12px] uppercase tracking-[0.14em] text-white/85">
                {activeTemplate?.name ?? "Choose a template"}
              </div>
              <div className="mt-1 font-mono text-[11px] leading-5 text-white/45">
                {activeTemplate?.description ?? "A short description will appear here to help you confirm the launch."}
              </div>
              ...
              <div className="mt-4 grid gap-3">
                ...
                <label className="flex flex-col gap-1">
                  <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Name override</span>
                  <input
                    value={nameOverride}
                    onChange={(event) => setNameOverride(event.target.value)}
                    placeholder={activeTemplate?.name ?? "Custom playbook name"}
                    className="rounded border border-white/10 bg-black/50 px-2 py-2 font-mono text-[11px] text-white/80 outline-none placeholder:text-white/20"
                  />
                </label>
              </div>

              <div className="mt-4 rounded border border-white/8 bg-white/[0.03] px-3 py-3">
                <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">Launch preview</div>
                <div className="mt-2 font-mono text-[11px] text-white/65">Template: {activeTemplate?.name ?? "None selected"}</div>
                <div className="mt-1 font-mono text-[11px] text-white/65">Agent: {selectedAgentName}</div>
                <div className="mt-1 font-mono text-[11px] text-white/65">Job name: {nameOverride.trim() || activeTemplate?.name || "—"}</div>
              </div>

              <button
                type="button"
                onClick={() => void handleCreate()}
                disabled={createBusy || !activeTemplate}
