...
const AgentsPageScreen = () => {
  ...
  const loadCompanyScopedAgentScope = useCallback(async (force = false): Promise<CompanyAgentScope> => {
    if (!force && companyScopedAgentIdsRef.current !== null && companyScopedAgentSlugsRef.current !== null) {
      return { gatewayAgentIds: companyScopedAgentIdsRef.current, agentSlugs: companyScopedAgentSlugsRef.current };
    }
  ...
  const loadAgents = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const companyAgentScope = await loadCompanyScopedAgentScope();
