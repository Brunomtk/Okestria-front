...
const formatAttachmentBytes = (sizeBytes: number): string => {
...
const AgentChatComposer = memo(function AgentChatComposer({
  ...
  const handleDragEnter = useCallback((event: DragEvent<HTMLDivElement>) => {
    if (!onFilesDropped || !canSend || running) return;
    if (!Array.from(event.dataTransfer?.types ?? []).includes("Files")) return;
    event.preventDefault();
    dragDepthRef.current += 1;
    setIsDragActive(true);
  }, [canSend, onFilesDropped, running]);
  ...
  }, [canSend, isDragActive, onFilesDropped, running]);
  ...
  }, [canSend, clearDragState, onFilesDropped, running]);
  ...
  return (
    <>
      ...
      <div
        className={`relative rounded-2xl border px-3 py-2 transition ${isDragActive ? "border-primary/70 bg-primary/10 shadow-[0_0_0_1px_rgba(255,255,255,0.08)]" : "border-border/65 bg-surface-2/45"}`}
      ...
      >
        {isDragActive ? (
          <div className="mb-2 rounded-xl border border-dashed border-primary/60 bg-card/70 px-3 py-2 text-center font-mono text-[11px] tracking-[0.02em] text-foreground">
            Drop files here to attach them to the message
          </div>
        ) : null}
        ...
        {attachmentError ? (
          <div
            className="mb-2 rounded-md border px-2.5 py-1.5 font-mono text-[10px] tracking-[0.02em] ui-badge-status-error"
        ...
        {hasAttachments ? (
          <div className="mb-2 flex flex-wrap gap-1.5" data-testid="agent-attachment-list">
            {attachments.map((attachment, index) => (
              <div
                key={`${attachment.filename}-${index}`}
                className="flex items-center gap-2 rounded-md border border-border/70 bg-card/80 px-2 py-1 text-[11px] text-foreground"
              >
                <Paperclip className="h-3 w-3 text-muted-foreground" />
                <div className="min-w-0">
                  <div className="max-w-[180px] truncate" title={attachment.filename}>{attachment.filename}</div>
                  <div className="text-[10px] text-muted-foreground">{formatAttachmentBytes(attachment.sizeBytes)}</div>
                </div>
                <button
                  type="button"
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            title={running ? "Wait for the current run to finish before sending files" : "Attach files"}
          >
...
export const AgentChatPanel = ({
  ...
  const handleAttachmentFiles = useCallback(async (files: File[]) => {
    ...
    const nextFiles = files.slice(0, Math.max(0, MAX_CHAT_ATTACHMENTS - attachments.length));
    let nextError: string | null = null;
    if (nextFiles.length !== files.length) {
      nextError = `You can attach up to ${MAX_CHAT_ATTACHMENTS} files at a time.`;
    }

    const existingTotal = attachments.reduce((sum, entry) => sum + entry.sizeBytes, 0);
    let runningTotal = existingTotal;
    const prepared: ChatSendAttachment[] = [];

    for (const file of nextFiles) {
      if (file.size > MAX_CHAT_ATTACHMENT_BYTES) {
        nextError = `${file.name} exceeds the ${formatAttachmentBytes(MAX_CHAT_ATTACHMENT_BYTES)} per-file limit.`;
        continue;
      }
      if (runningTotal + file.size > MAX_CHAT_ATTACHMENT_TOTAL_BYTES) {
        nextError = `Attachments exceed the ${formatAttachmentBytes(MAX_CHAT_ATTACHMENT_TOTAL_BYTES)} total limit.`;
        continue;
      }
      ...
      <div className="mt-3 flex min-h-0 flex-1 flex-col px-3 pb-3 sm:px-4 sm:pb-4">
        ...
        <div className="mt-3">
          ...
          <AgentChatComposer
            ...
            onPickAttachment={handleAttachmentPicker}
            onFilesDropped={handleAttachmentFiles}
