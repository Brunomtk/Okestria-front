...
export async function sendChatMessageViaStudio(params: {
  ...
  const trimmed = params.payload.text.trim();
  const attachments = Array.isArray(params.payload.attachments)
    ? params.payload.attachments
        .filter((entry) => entry && typeof entry.filename === "string" && entry.filename.trim())
        .map((entry) => ({
  ...
  params.dispatch({
    ...
    patch: {
      ...
      thinkingTrace: null,
      draft: "",
      ...(echoUserMessage && trimmed ? { lastUserMessage: trimmed } : {}),
  ...
  if (echoUserMessage) {
    const attachmentSummary =
      attachments.length > 0
        ? `Attached: ${attachments.map((entry) => entry.filename).join(", ")}`
        : "";
    const echoedLines = [trimmed, attachmentSummary].filter((value) => value.trim().length > 0);
    if (echoedLines.length > 0) {
      params.dispatch({
        type: "appendOutput",
        agentId,
        line: echoedLines.map((line) => `> ${line}`).join("\n"),
        transcript: {
          source: "local-send",
          runId,
