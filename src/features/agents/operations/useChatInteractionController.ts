...
export type ChatInteractionController = {
  ...
  handleSend: (agentId: string, sessionKey: string, payload: ChatSendPayload) => Promise<void>;
      ...
      if (agent.status === "running") {
        if (attachments.length > 0) {
          params.dispatch({
            type: "appendOutput",
            agentId,
            line: "Error: Wait for the current run to finish before sending files.",
          });
          return;
        }
        params.dispatch({
