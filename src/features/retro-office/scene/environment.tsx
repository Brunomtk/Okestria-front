...
export const FloorAndWalls = memo(function FloorAndWalls({
  ...
  const [qaZoneCenterX, , qaZoneCenterZ] = toWorld(
    QA_LAB_X + QA_LAB_WIDTH / 2,
    QA_LAB_TOP_Y + EAST_WING_ROOM_HEIGHT / 2,
  );
  ...
  const hallZoneWidth = Math.max(0, EAST_HALL_WIDTH * SCALE);
  ...
  const hallZoneFloorWidth = Math.max(0, hallZoneWidth - roomFloorInset * 2);
  const [hallZoneCenterX] = toWorld(
    GYM_ROOM_END_X + EAST_HALL_WIDTH / 2,
    EAST_WING_ROOM_TOP_Y + EAST_WING_ROOM_HEIGHT / 2,
  );
  ...
  return (
    <group>
      ...
      {/* Hall floor between gym and QA lab */}
      {hallZoneFloorWidth > 0 && roomZoneFloorHeight > 0 ? (
        <>
          <mesh
            position={[hallZoneCenterX, 0.002, roomZoneCenterZ]}
            rotation={[-Math.PI / 2, 0, 0]}
            receiveShadow
          >
            <planeGeometry args={[hallZoneFloorWidth, roomZoneFloorHeight]} />
            <meshStandardMaterial
              color="#1a1d22"
              roughness={0.95}
              metalness={0.05}
          ...
          {showRemoteOffice ? (
            <mesh
              position={[hallZoneCenterX, 0.002, roomZoneCenterZ + remoteOfficeOffsetZ]}
              rotation={[-Math.PI / 2, 0, 0]}
              receiveShadow
            >
              <planeGeometry args={[hallZoneFloorWidth, roomZoneFloorHeight]} />
      ...
      {qaZoneFloorWidth > 0 && roomZoneFloorHeight > 0 ? (
        <>
          <mesh
            position={[qaZoneCenterX, 0.003, qaZoneCenterZ]}
            rotation={[-Math.PI / 2, 0, 0]}
            receiveShadow
          >
          ...
          {showRemoteOffice ? (
            <mesh
              position={[qaZoneCenterX, 0.003, qaZoneCenterZ + remoteOfficeOffsetZ]}
              rotation={[-Math.PI / 2, 0, 0]}
              receiveShadow
            >
              <planeGeometry args={[qaZoneFloorWidth, roomZoneFloorHeight]} />
          ...
          <mesh
            position={[qaZoneCenterX, 0.004, qaZoneCenterZ]}
            rotation={[-Math.PI / 2, 0, 0]}
            receiveShadow
          >
          ...
          {showRemoteOffice ? (
            <mesh
              position={[qaZoneCenterX, 0.004, qaZoneCenterZ + remoteOfficeOffsetZ]}
              rotation={[-Math.PI / 2, 0, 0]}
              receiveShadow
            >
              <planeGeometry args={[qaZoneFloorWidth * 0.96, roomZoneFloorHeight * 0.88]} />
            ...
            return (
              <group key={`qa-vertical-group-${index}`}>
                <mesh
                  ...
                  position={[offsetX, 0.006, qaZoneCenterZ]}
                  rotation={[-Math.PI / 2, 0, 0]}
                >
                ...
                {showRemoteOffice ? (
                  <mesh
                    ...
                    position={[offsetX, 0.006, qaZoneCenterZ + remoteOfficeOffsetZ]}
                    rotation={[-Math.PI / 2, 0, 0]}
                  >
                    <planeGeometry args={[0.015, qaZoneStripeHeight]} />
                    <meshBasicMaterial color="#7c3aed" transparent opacity={0.34} />
