...
export const updateCompanySquad = async (params: {
  ...
  const catalog = await fetchSquadCatalog({ companyId, token: params.token });
  const dedupedMemberAgentIds = [...new Set(params.memberAgentIds.filter((v) => Number.isFinite(v) && v > 0))];
  const selectedCatalogAgents = catalog.agents.filter((a) => dedupedMemberAgentIds.includes(a.agentId));
  ...
  await requestBackendJson<unknown>(
    `/api/Squads/update/${params.squadId}`,
    {
      ...
      body: JSON.stringify({
        ...
        defaultExecutionMode: normalizeExecutionMode(params.executionMode),
        isActive: true,
        members: selectedCatalogAgents.map((agent, index) => ({
          agentId: agent.agentId,
          order: index,
          isLeader: agent.agentId === leaderAgentId,
          canReceiveTasks: agent.status === true,
  ...
  const refreshed = await fetchCompanySquads({ companyId, token: params.token });
  const matched = refreshed.find((entry) => Number(entry.id) === params.squadId);
  if (!matched) {
    throw new Error("The squad was updated, but the refreshed list did not return it.");
  }
  return matched;
};
