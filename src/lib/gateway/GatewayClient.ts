    ...
    const nextClient = new GatewayBrowserClient({
      ...
      onClose: ({ code, reason }) => {
        ...
        const err = connectFailed
          ...
          : new Error(
              code === 1011 && reason.trim().toLowerCase() === "connect failed"
                ? "Could not connect to the gateway. Check the gateway URL, token/device auth, and whether the gateway is online."
      ...
      try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (cancelled) return;
            if (data.configured && data.gatewayUrl) {
              const nextGatewayUrl = data.gatewayUrl.trim();
        ...
        // Fallback: Load settings from local settings coordinator
        const envelope =
